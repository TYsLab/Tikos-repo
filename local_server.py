...
BRACKET_PATH  = Path(__file__).parent / "bracket_results.json"
BRACKET_TTL   = 6 * 3600  # 6 hours in seconds
...
def _bracket_cache_valid() -> bool:
    """Return True if bracket_results.json exists and is less than 6 hours old."""
    if not BRACKET_PATH.exists():
        return False
    age = datetime.now().timestamp() - BRACKET_PATH.stat().st_mtime
    return age < BRACKET_TTL


def get_bracket_results() -> dict:
    """Return cached bracket results if fresh, otherwise run fresh predictions."""
    if _bracket_cache_valid():
        with open(BRACKET_PATH) as f:
            return json.load(f)
    return _run_bracket_sync()


def _run_bracket_sync() -> dict:
    """Run bracket_predictor_v2 (advanced) with fallback to v1."""
    import sys, importlib
    sys.path.insert(0, str(Path(__file__).parent))
    # Try advanced v2 first
    try:
        import bracket_predictor_v2
        importlib.reload(bracket_predictor_v2)
        return bracket_predictor_v2.run_predictions()
    except Exception as exc:
        print(f"  v2 predictor error: {exc} — falling back to v1")
    # Fallback to original
    try:
        import bracket_predictor
        importlib.reload(bracket_predictor)
        return bracket_predictor.run_predictions()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ('/', '/index.html', '/bracket.html', '/craps.html'):
            file = self.path.lstrip('/') or 'index.html'
            content = (Path(__file__).parent / file).read_bytes()
            self.send_response(200)
        ...
        if self.path == '/api/bracket':
            self.send_json(get_bracket_results())
            return
    ...
    def do_POST(self):
        ...
        if self.path == '/api/run-bracket':
            # Force a fresh bracket prediction run (ignore cache)
            results = _run_bracket_sync()
